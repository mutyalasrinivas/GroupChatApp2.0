...
document.addEventListener('DOMContentLoaded', async (e) =>{
    let listOfGroups = await axios.get('http://localhost:2000/group/getUserGroups');
...
document.querySelector('#newGroupBtn').addEventListener('click', async () => {
    ...
    let usersList = await axios.get('http://localhost:2000/user/getUsers');
...
document.querySelector('#createGroupForm').addEventListener('submit', async (e) => {
    ...
    const createGroup = await axios.post('http://localhost:2000/group/createGroup', {
        name: groupName,
        users: groupUsers
    });
    await axios.post('http://localhost:2000/admin/makeGroupAdmin',{
...
async function addGroupToSideBar(group){
    ...
    let groupData = await axios.get(`http://localhost:2000/message/lastMessage?groupId=${group.id}`);
...
async function showGroup(id, name){
    ...
    const checkGroupAdmin = await axios.get(`http://localhost:2000/admin/checkAdmin?groupId=${id}`);
    if(checkGroupAdmin.data){
        const adminBtn = document.createElement('button');
        adminBtn.setAttribute('class', 'btn btn-link');
        adminBtn.setAttribute('style', 'display: inline; margin-left: 20px;  ');
...
document.querySelector('#sendMessage').addEventListener('click', async () => {
    ...
    await axios.post('http://localhost:2000/message/newGroupMessage', {
        message: message,
        chatGroupId: groupId
    })
...
fileInput.addEventListener('change', async () => {
    ...
    await axios.post('http://localhost:2000/message/saveFile', {
        file: selectedFile,
        chatGroupId: groupId
    })
})
...
async function fetchMessages(id){
    const groupMessagesBox = document.querySelector('.groupMessagesBox');
    let messages = await axios.get(`http://localhost:2000/message/getGroupMessages/${id}`);
...
async function fetchNewMessages(id, groupMessagesBox, userId){
    let newMessages = await axios.get(`http://localhost:2000/message/getMessages?groupId=${id}&lastMessage=${lastMessageId}`);
...
async function showGroupParticipants(id){
    ...
    let allUsers = await axios.get('http://localhost:2000/user/getUsers');
    ...
    allUsers.forEach(async user => {
        ...
        const isParticipant = await axios.get(`http://localhost:2000/group/checkGroupUser?groupId=${id}&userId=${user.id}`);
        if(isParticipant.data){
            const isAdmin = await axios.get(`http://localhost:2000/admin/checkAdmin?groupId=${id}&userId=${user.id}`)
            if(isAdmin.data){
                removeAdmin.innerHTML = 'Remove Admin';
                column1.appendChild(removeAdmin);
                removeAdmin.addEventListener('click', async () => {
                    await axios.post('http://localhost:2000/admin/removeAdmin',{
                        chatGroupId: id,
                        userId: user.id
                    });
                });
                ...
                addAdmin.addEventListener('click', async () => {
                    await axios.post('http://localhost:2000/admin/makeNewAdmin',{
                        chatGroupId: id,
                        userId: user.id
                    });
                });
            ...
            removeButton.addEventListener('click', async () => {
                await axios.post('http://localhost:2000/group/removeFromGroup',{
                    chatGroupId: id,
                    userId: user.id
                });
            })
            ...
            addToButton.addEventListener('click', async () => {
                await axios.post('http://localhost:2000/group/addNewGroupUser', {
                    chatGroupId: id,
                    userId: user.id
